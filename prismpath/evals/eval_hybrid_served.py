...
total_cases = len(recs); multi = [r for r in recs if r[6] > 1]
print(f"=== arms on {total_cases} cases ({len(multi)} multi-edge) | LLM=served gemma4 ===")
print(f"  EMBED-only : acc={sum(r[3]==r[2] for r in recs)/total_cases:.2f}  (0 LLM calls)")
print(f"  LLM-only   : acc={sum(r[5]==r[2] for r in recs)/total_cases:.2f}  ({len(multi)} LLM calls)")
