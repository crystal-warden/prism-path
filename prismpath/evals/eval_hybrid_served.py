...
from prismpath.routing.router import EmbeddingRouter, LLMRouter
from prismpath.evals.eval_routing import CASES

def served_generate(prompt):
    response = requests.post("http://127.0.0.1:8888/v1/chat/completions",
...
for node, outcome, expected in CASES:
    edges = graph.nodes[node].edges; instr = graph.nodes[node].instruction
...
total_cases = len(recs); multi = [record for record in recs if record[6] > 1]
print(f"=== arms on {total_cases} cases ({len(multi)} multi-edge) | LLM=served gemma4 ===")
print(f"  EMBED-only : acc={sum(record[3]==record[2] for record in recs)/total_cases:.2f}  (0 LLM calls)")
print(f"  LLM-only   : acc={sum(record[5]==record[2] for record in recs)/total_cases:.2f}  ({len(multi)} LLM calls)")
...
for delta in (0.03,0.05,0.08,0.12,0.20,0.30):
    ...
    for (_n,_o,exp,et,margin,lt,ne) in recs:
        pick = lt if (ne>1 and margin<delta) else et
        if ne>1 and margin<delta: calls+=1
        ok += pick==exp
    print(f"  {delta:6.2f} {ok/total_cases:5.2f} {calls:8d} {calls/len(multi):7.0%}")
