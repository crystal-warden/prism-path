...
**upgrade** promotes it to a full Bitcoin proof after the aggregating tx confirms (~1-6 h).
...
2. **Batch-and-forward** — accumulate roots in-enclave; carry hashes out through the diode/maintenance
...
Only after stamp→upgrade→verify round-trips AND the air-gap tier ships may the pitch say
...
- **Anchor at ingestion, not just at verdict.** Hash raw evidence at the sensor/export boundary and
  ...
  sensor→first-anchor latency, which §3.1's fast tick minimizes.
...
Between stamp and Bitcoin confirmation (~1-6 h) you hold only the pending calendar attestation.
**Compensate:**
...
A proof over a hash is useless without the content to re-hash.
...
**re-stamp** long-lived proofs periodically so trust doesn't rest on a single chain-of-custody.
...
3. `prismpath ledger verify --ots` + tamper-test release gate.  *(~1-2 days)*
4. **Air-gap tier** (egress-stamp / batch-forward / RFC-3161 fallback) — the hard, high-value piece.  *(~1-2 wk)*
5. Ingestion-boundary anchoring + `POLICY_HASH` binding (the C1 compensations).  *(~2-3 days)*
Connected v1 (1-3) is ~a week; the defensible IP is item 4.
