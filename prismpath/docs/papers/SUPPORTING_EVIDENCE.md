...
> **Rule of use.** No claim ships without its row here (result + provenance + gap). Reproducers are
> scripts under `mdflow/` (working tree) writing JSON under `mdflow/benchmark/` and
> `etbert-lab/benign_corpus/eval/`.
...
| Stratified router (g_diag on lint edges), #47 | polarity lint fires on **0/99** cases → g_diag has no delivery mechanism | DEAD ON ARRIVAL | `/tmp/lint_falsify.py` |
...
## E. Encrypted-traffic detection — margin + calibration (#34; CONTRIB_outline_research)
| Claim | Result | Provenance |
|---|---|---|
...
| GPU ingestion (measured) | log embed CPU 179 → GPU **2,473 ev/s = 13.8×**; flow encoder ~13×; client-year in ~8 GPU-hours | `/tmp/log_embed_bench.py`; SERVICE_CLAIMS D7 |
...
|---|---|---|
| ET-BERT capture redesign (#33) | silent-failure war-story; no-sudo rotating capture; hourly GPU batch (4.23 ms/flow); cwcheck function-check | `CONTRIB_outline_engineering`; `batch_score.py` |
| Per-tenant suppression (#44) | **17.3% hard-suppressed** (skip LLM) on 5k live alerts + context-tagged over-calls; hash-versioned | `mdflow/suppression.py`, `homelab.json` |
| Learning flywheel (#38) | authoritative labels teach cache (recurrence auto-resolves cos 0.994) + propose context rules; **self-labels REFUSED, threats never suppressed** | `mdflow/flywheel.py` |
...
| Shadow-pilot harness (P6) | dry-run: containment agreement 0.867, **0 dangerous under-calls**, 0 parse errors | `mdflow/shadow_agreement.py` |
...
| Knowledge-library nodes (#58): retrieval-augmented adjudication — mechanism+principle win, efficacy data-gated | Real 239-card LOLBAS index (EmbeddingGemma 768d), retrieval precise (14/64 malicious, **0/48 benign** false-retrieval), air-gap Docker image, KB-hash bound into Flow-Ledger provenance (#53). VALIDATION: v0 injected the full card incl. its 'legitimate binary, mere presence not malicious' framing → HURT the escalation node (recall 0.969->0.875, the #1/#2/#3 dilution effect at a 3rd layer). v1 ONE-DIRECTIONAL abuse-only injection → RECOVERED (ER 0.969/0.938 = E no-knowledge, no harm) + lifted a NEUTRAL node 0.844->0.906 + fixed the wuauclt under-call WITH a cited justification (audit value). **Efficacy WIN not established on this corpus** — knowledge is redundant with the escalation-default prior for recall, and its unique PRECISION benefit (correctly ignoring genuinely-benign LOLBin use) is UNTESTABLE here (0 benign LOLBin cases). Design rule: inject one-directional abuse-evidence, never balanced prose. | `triage-corpus/rag_nodes_v1.json`, `mdflow/{build_knowledge_index,validate_triage_rag}.py`, `knowledge-lib/` |

## H. Document map (the whole set, navigable)
- **Papers:** `prismpath_paper_research.md` (research), `prismpath_whitepaper_engineering.md` (engineering).
- **Merge-ready outlines:** `CONTRIB_outline_research_etbert_margin.md`, `CONTRIB_outline_engineering_etbert_hardening.md`.
- **Specs:** `SPEC_ledger_opentimestamps.md` (OTS, connected-v1 delivered), `etbert-lab/HARDENING_SPEC_benign_margin.md`.
- **Parking lot / negative results:** `RESEARCH_IDEAS_parking_lot.md` (density thread PARKED, hyperbolic/curvature parked).
- **On-ramp:** `PRIMER_a_students_guide.md`.
- **Claims + strategy (private, cw-strategy/):** `SERVICE_CLAIMS.md`, `BRAND_ARCHITECTURE.md`, `CONSULTING_STRATEGY.md`, `RETROSPECTIVE.md`, `CAPACITY.md`.
- **Detection claim discipline:** `etbert-lab/CLAIMS_detection_metrics.md`.
- **Use case:** `MDFLOW_USECASE_blue_team_soc_triage.md`.

## I. Reproducibility
Scripts (working tree `~/cwprojects/mdflow/` + `etbert-lab/`): `gaussian_route_eval.py`, `gaussian_route_pca.py`,
