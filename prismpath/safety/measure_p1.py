...
the point of P1 being optional). Run it where one exists:
    ...
    PYTHONPATH=. <venv-with-sentence-transformers>/bin/python -m prismpath.safety.measure_p1
...
def main() -> int:
    ...
    floor_path = Path(__file__).parent.parent / "policies" / "statutory_floor.md"
    ...
    out = Path(__file__).parent.parent / "measurements" / "p1_sweep.json"
