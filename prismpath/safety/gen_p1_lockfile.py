...
needs to run P1 with the behaviour that was actually MEASURED, and nothing it could vary:
  ...
  * the **embedder identity**, including runtime and precision, because the same weights under a
    ...
    python -m prismpath.safety.gen_p1_lockfile            # write the lockfile
    python -m prismpath.safety.gen_p1_lockfile --check    # verify the committed one is current
...
OUT_PATH = Path(__file__).parent.parent / "policies" / "p1_lockfile.json"
CORPUS = Path(__file__).parent.parent / "portable" / "conformance" / "safety.json"
...
def generate() -> dict:
    from prismpath.safety.guard_semantic import PROHIBITED_INTENT_EXEMPLARS, SemanticLayer
    ...
    from prismpath.safety.bypass_corpus import SEMANTIC_VARIANTS
