...
def test_float_ordering_constant_is_refused():
    with pytest.raises(ValueError, match="float"):
        q.build_partitions(_graph("-> a: when x > 21.7\n-> b: else"))


def test_float_equality_constant_is_refused():
    with pytest.raises(ValueError, match="float"):
        q.build_partitions(_graph("-> a: when x == 2.5\n-> b: else"))


def test_float_in_list_member_is_refused():
    with pytest.raises(ValueError, match="float"):
        q.build_partitions(_graph("-> a: when x in (3, 5.5)\n-> b: else"))


def test_integer_thresholds_still_partition():
    parts = q.build_partitions(_graph("-> a: when x > 21\n-> b: else"))
