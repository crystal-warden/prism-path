...
a3_vectors.json). PrismPath's A3 result files are written by `groupa/cross_substrate_decisions.py` only after an MCU leg
...
4. Then `python -m prismpath.comparisons.groupa.cross_substrate_decisions --write-prismpath` (to be added: it should read the
...
loaded once on the board by `groupa/fabric_leg_run.py` (which also repeated the A3 leg, 23/23), then
`groupa/wcet_sweep_only.py <policy> <seconds>` attached to the resident overlay without reconfiguring and
swept that policy's corpus readings continuously through the PS evaluate path, while on gx10
`groupa/wcet_pins_witness.py --policy <id> --bound <wcet> --iters 30` took 30 free run captures on Pmod JB with
...
board survived, but kill by PID at once. `groupa/bounded_decision_time.py` reads `pins_<policy>.json` and grades NATIVE
