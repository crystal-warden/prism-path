...
def loc(paths) -> int:
    line_count = 0
    for path in paths:
        for line in (GLUE_DIR / path).read_text().splitlines():
            stripped = line.strip()
...
class Board:
    def __init__(self, port: str):
        self.serial_port = serial.Serial(port, 115200, timeout=15)
        time.sleep(1.0)
        self.serial_port.reset_input_buffer()

    def frame(self):
        tag = self.serial_port.read(1)
        if len(tag) != 1:
            raise RuntimeError("serial timeout")
        length = struct.unpack("<H", self.serial_port.read(2))[0]
        return tag, self.serial_port.read(length)

    def ident(self) -> str:
        self.serial_port.write(b"I"); tag = self.serial_port.read(1); length = self.serial_port.read(1)
        assert tag == b"i"
        return self.serial_port.read(length[0]).decode()

    def stats(self) -> dict:
        self.serial_port.write(b"S"); tag, body = self.frame(); assert tag == b"s"; return json.loads(body)

    def load(self, idx: int) -> str:
        self.serial_port.write(b"L" + bytes([idx])); tag, body = self.frame()
        if tag != b"l":
            raise RuntimeError(f"load failed: {body!r}")
        return body.decode()

    def eval(self, inp: dict):
        body = json.dumps(inp, separators=(",", ":")).encode()
        t0 = time.perf_counter()
        self.serial_port.write(b"V" + struct.pack("<H", len(body)) + body)
        tag, response = self.frame()
        dt = (time.perf_counter() - t0) * 1e6
        return tag, response, dt
...
def main(argv=None) -> int:
    ...
    for idx, pid in enumerate(POLICIES):
        ...
        for sid, kind, inp, exp in scenario_steps(policy):
            ...
            tag, r, dt = board.eval({field: value for field, value in inp.items() if value is not None})
            if tag == b"M":
    ...
    stats = board.stats()
    n_ok = sum(1 for row in rows if row["agree"])
    ...
    record = {"ident": ident, "isa": isa, "wasm3_commit": wasm3_commit(), "stats_after": stats, "rows": rows, "agree": n_ok, "total": len(rows),
              "glue_loc": glue_loc, "glue_files": GLUE_FILES, "hours": args.hours}
    ...
    within = glue_loc <= 300 and args.hours <= 8
    ...
    glue = {"description": ("OPA's own compiler (opa build -t wasm) plus the wasm3 WebAssembly interpreter (third party, MIT) built into "
            ...
            "loc": glue_loc, "hours": args.hours}
    # every ISA leg recorded so far (opa_wasm3_<isa>.json) rides along in the result files
    legs = {}
    for leg_path in sorted(ev.glob("opa_wasm3_*.json")):
        rec = json.loads(leg_path.read_text())
    ...
    for pid in POLICIES:
        for sid, kind, inp, exp in scenario_steps(policy_by_id(pid)):
            ...
            write_result(system="opa+glue", dimension="A3", policy=pid, scenario=sid, expected=exp["outcome"],
                         observed=row["board_outcome"], grade=grade, idiomatic=True, evidence_path=ev, glue=glue,
                         measurements={"round_trip_us": row["round_trip_us"], "heap_high_water_bytes": stats["heap_high_water"],
                                       "linear_memory_bytes": stats["linear_memory_bytes"], "isa": isa, "legs": legs},
                         notes=(f"OPA's WebAssembly module for this policy decided {row['board_outcome']}/{row['board_rule']} on the RP2350 "
                                f"({isa}) under wasm3, expected {exp['outcome']}/{exp.get('rule')}: {'identical' if row['agree'] else 'DIFFERENT'}. "
                                ...
                                f"{args.hours} h, {'within' if within else 'OVER'} the section 4 budget, no trust anchor added. Memory on the board: "
                                f"{stats['heap_high_water']} B heap high water of 520 KB SRAM, {stats['linear_memory_bytes']} B linear memory, "
                                f"{row['round_trip_us']:.0f} us USB round trip{' (first evaluation after load, lazy compile)' if row['first_eval_after_load'] else ''}. "
