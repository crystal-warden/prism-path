...
def _committed(system: str) -> dict:
    path = repo_file("prismpath", "comparisons", "systems", system, "generated", "conformance.json")
...
def test_prismpath_translator_conforms():
    ...
    assert rep["summary"]["MISMATCH"] == 0, [row for row in rep["rows"] if row["class"] == "MISMATCH"]
    dropped = sorted({row["expected"]["rule"] for row in rep["rows"] if row["class"] == "DROPPED"})
    assert dropped == ["r3", "r4"], "only the pre registered inexpressible B1 rules may be dropped"
    stuck = [row for row in rep["rows"] if row["policy"] == "sensor_interlock" and row["kind"] == "undeclared_missing"]
    assert stuck and all(row["observed"]["observed"] == "no_match" and row["observed"]["cause"] == 36 for row in stuck)
    human = [row for row in rep["rows"] if row["scenario"] == "escalate_human_requested_1"]
...
def test_prismpath_generated_flows_are_level_m():
    ...
    for meta in (SYSTEMS_DIR / "prismpath" / "generated").glob("*/TRANSLATION.json"):
        ...
        if metadata["expressible"]:
            assert "Level M: yes" in metadata["notes"], metadata["policy"]
...
def test_every_translation_carries_citations_and_notes():
    ...
    for meta in SYSTEMS_DIR.glob("*/generated/*/TRANSLATION.json"):
        metadata = json.loads(meta.read_text(encoding="utf-8"))
        assert metadata["citations"], meta
        assert metadata["notes"], meta
        assert isinstance(metadata["idiomatic"], bool)
