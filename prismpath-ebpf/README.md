...
| Bounds tuning (64→16→8) | The state count is **insensitive** to the loop bounds: it plateaus at ~43k to 47k regardless. 64×64 hit the 8192-jump limit; 16×16 processed exactly 1,000,001 insns (1 over); 8×8 *unrolled* was **worse** (68k states). |
...
- **Latency (kernel-measured, `BPF_PROG_TEST_RUN` × 1e6):** **132 to 182 ns/packet** for parse + Level M
  eval, **~5.5 to 7.6 Mpps on one core** (aarch64, generic/SKB XDP). Sub-microsecond; measured, not asserted.
