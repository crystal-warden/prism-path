...
port, names_path, log_path = int(sys.argv[1]), sys.argv[2], sys.argv[3]
names = json.load(open(names_path))["names"]
...
while not stop:
    ...
    if len(pkt) < 12:
        odd += 1
        log.write(f"odd short {len(pkt)} B\n")
        continue
    ...
    if tag is None and node_idx == 0xFFFFFFFE:
        undecided += 1
        log.write(f"truncated packet ({len(pkt)} B, claims {n_fields} fields): kernel refused-short\n")
        log.flush()
        continue
    if magic != 0x4D545050:
        odd += 1
        log.write(f"not a PPT packet (magic {magic:#x}): passed untouched, undecided, {len(pkt)} B\n")
        log.flush()
        continue
    kernel = (
        names[node_idx]
    ...
    if tag == 1:  # a deliberately malformed sender packet (n_fields 0 or truncated registers)
        undecided += 1 if kernel in ("no-match", "refused-short") else 0
        log.write(f"seq {seq} malformed by design (n_fields {n_fields}, {len(pkt)} B): kernel {kernel}\n")
        log.flush()
        continue
    if kernel == relay:
        agree += 1
    ...
    log.flush()
log.write(
    f"# summary: agree {agree}, disagree {disagree}, malformed decided no-match {undecided}, not PPT {odd}, {time.time() - t0:.0f} s\n"
)
log.close()
