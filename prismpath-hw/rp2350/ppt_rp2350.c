...
static uint8_t rx(void) {                        /* block until one byte arrives */
    int byte;
    while ((byte = getchar_timeout_us(1000000)) < 0) { tight_loop_contents(); }
    return (uint8_t)byte;
}
static void tx(uint8_t byte) { putchar_raw((char)byte); }
...
static int32_t rd32(const uint8_t *bytes) {
    int32_t value;
    memcpy(&value, bytes, 4);           /* little-endian, same as the format */
    return value;
}
