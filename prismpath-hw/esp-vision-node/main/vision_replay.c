...
static void front_end(int32_t *motion_cells, int32_t *dark, int32_t *step, int32_t *door_hit) {
    ...
    *door_hit = dh;
    frame_n++;
    if (frame_n % REFRESH == 0 && mc == 0) memcpy(background, cur, W * H);
    memcpy(prev, cur, W * H);
}
...
void app_main(void)
{
    cur = heap_caps_malloc(W * H, MALLOC_CAP_SPIRAM); prev = heap_caps_malloc(W * H, MALLOC_CAP_SPIRAM); background = heap_caps_malloc(W * H, MALLOC_CAP_SPIRAM);
    memcpy(tbl, POLICY_TABLE, POLICY_TABLE_LEN);
    uint8_t rc = parse_table(POLICY_TABLE_LEN);
    if (rc) { ESP_LOGE(TAG, "table parse failed rc=%u", rc); while (1) vTaskDelay(1000); }
    ...
    while (1) {
        ...
        for (int r = 0; r < R; r++) for (int c = 0; c < C; c++) { /* registers by generated name */ }
        ...
        memset(wirebuf, 0, sizeof wirebuf); bitacc_t acc = { wirebuf, 0 };
        for (int i = 0; i < WIRE_N_FIELDS; i++) {
            int32_t v = get_reg(WIRE_FIELDS[i].reg); fields[i] = v; uint32_t sym = 0;
            for (int k = 0; k < WIRE_FIELDS[i].n_cuts; k++) if (v >= WIRE_FIELDS[i].cut[k]) sym = k + 1;
            zeck_encode(&acc, (uint64_t)sym + 1);
        }
        uint16_t wire_len = (uint16_t)((acc.bitpos + 7u) >> 3);
