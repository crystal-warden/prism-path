...
typedef struct __attribute__((packed)) {
    char magic[4]; uint32_t seq; uint64_t t_cap_us, t_send_us; uint16_t width, height; uint32_t len;
} frame_hdr_t;
...
{
    while (remaining) {
        size_t piece = remaining < USB_PIECE ? remaining : USB_PIECE;
        int written = usb_serial_jtag_write_bytes(bytes, piece, pdMS_TO_TICKS(1000));
        if (written <= 0) { vTaskDelay(1); continue; }
        bytes += written; remaining -= written;
    }
}
...
static bool send_frame(uint32_t seq, bool fresh)
{
    if (fresh) {   // a single frame on request must be current: drain what the driver queued while idle
        for (int drain_index = 0; drain_index < 3; drain_index++) { camera_fb_t *old = esp_camera_fb_get(); if (old) esp_camera_fb_return(old); }
    }
    camera_fb_t *fb = esp_camera_fb_get();
    if (!fb) return false;
    uint64_t t_cap = (uint64_t)fb->timestamp.tv_sec * 1000000ULL + (uint64_t)fb->timestamp.tv_usec;
    frame_hdr_t header = { {'F','R','M','2'}, seq, t_cap, (uint64_t)esp_timer_get_time(), (uint16_t)fb->width, (uint16_t)fb->height, (uint32_t)fb->len };
    usb_write_all((const uint8_t *)&header, sizeof header);
...
void app_main(void)
{
    ...
    while (1) {
        ...
        if (usb_serial_jtag_read_bytes(&ch, 1, 0) == 1) {
            ...
            else if (ch == 'x') { streaming = false; double elapsed_sec = (esp_timer_get_time() - t0) / 1e6; ESP_LOGI(TAG, "stream stop: %lu frames in %.1f s = %.2f fps", (unsigned long)sent, elapsed_sec, sent / elapsed_sec); }
