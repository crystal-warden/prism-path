...
#define SETM(r, c)                           \
    ...
    SETM(0, 1) SETM(0, 2) SETM(0, 3) SETM(0, 4) SETM(0, 5) SETM(0, 6) SETM(0, 7) SETM(1, 0) SETM(1, 1) SETM(1, 2)
        SETM(1, 3) SETM(1, 4) SETM(1, 5) SETM(1, 6) SETM(1, 7) SETM(2, 0) SETM(2, 1) SETM(2, 2) SETM(2, 3) SETM(2, 4)
            SETM(2, 5) SETM(2, 6) SETM(2, 7) SETM(3, 0) SETM(3, 1) SETM(3, 2) SETM(3, 3) SETM(3, 4) SETM(3, 5)
                SETM(3, 6) SETM(3, 7) SETM(4, 0) SETM(4, 1) SETM(4, 2) SETM(4, 3) SETM(4, 4) SETM(4, 5) SETM(4, 6)
                    SETM(4, 7) SETM(5, 0) SETM(5, 1) SETM(5, 2) SETM(5, 3) SETM(5, 4) SETM(5, 5) SETM(5, 6) SETM(5, 7)
#undef SETM
...
static void codebook_from_header(void) {
    ...
    for (int field_index = 0; field_index < WIRE_N_FIELDS; field_index++) {
        ...
        for (int cut_index = 0; cut_index < CB_MAX_CUTS; cut_index++) {
            wire_fields[field_index].cut[cut_index] =
                cut_index < WIRE_MAX_CUTS ? WIRE_FIELDS[field_index].cut[cut_index] : CUT_UNSET_FILL;
        }
    }
    esc_mask = 0;
    for (int node_index = 0; node_index < POLICY_N_NODES; node_index++) {
        const char *node_name = POLICY_NODE_NAMES[node_index];
        if (!strcmp(node_name, "tamper") || !strcmp(node_name, "evidence") || !strcmp(node_name, "door") ||
            !strcmp(node_name, "scene_changed")) {
