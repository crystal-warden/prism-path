...
#define KEY_RESEND_US 60000000ULL   /* Resend the background keyframe every minute for late-joining receivers (WIRE.md). */
...
static void send_keyframe(camera_fb_t *fb, uint32_t seq, uint64_t t_cap)
{
    ...
    ESP_LOGI(TAG, "keyframe %lu B in %u fragments, normal %04x",
             (unsigned long)(sizeof(kh) + jlen),
             (unsigned)((sizeof(kh) + jlen + FRAG_DATA - 1) / FRAG_DATA),
             normal_id);
}
...
static uint16_t swap_execute(uint32_t *out_version, uint64_t *out_hash, uint32_t *out_us)
{
    ...
    if (staged_total == 0 || staged_count != staged_total) {
    ...
    } else if (staged_len < PACK_HDR_LEN ||
               memcmp(staged, "PPKV1", PACK_MAGIC_LEN) != 0 ||
        ...
        if ((uint32_t)PACK_HDR_LEN + image_len + cb_len != staged_len ||
            cb_len % CODEBOOK_FIELD_LEN != 0 ||
            cb_len / CODEBOOK_FIELD_LEN > CB_MAX_FIELDS ||
            image_len > TBL_MAX) {
            cause = CAUSE_MANIFEST_BAD_FORMAT;
        } else {
            bool zero = true;
            ...
            if (zero) {
                ...
                if (crypto_ed25519_check(staged + PACK_SIG_OFF, AUTHORITY_PUBKEY, vmsg, PACK_SIG_OFF + image_len + cb_len) != 0) {
                    ...
                    if (parse_table(image_len) != 0) {
                        ...
                        for (int i = 0; i < wire_n; i++) {
                            ...
                            for (int k = 0; k < CB_MAX_CUTS; k++) {
                                wire_fields[i].cut[k] = rd32(codebook + CODEBOOK_FIELD_LEN * i + CODEBOOK_CUT_BASE_OFF + CODEBOOK_CUT_BYTES * k);
                            }
                        }
                        esc_mask = esc;
...
static uint16_t act_execute(uint32_t *out_counter)
{
    uint16_t cause = CAUSE_OK;
    uint32_t counter = 0;
    if (staged_total == 0 || staged_count != staged_total || staged_len != ACT_PACK_LEN || memcmp(staged, "ACT1", ACT_MAGIC_LEN) != 0) {
        cause = CAUSE_MANIFEST_BAD_FORMAT;
    } else {
...
void app_main(void)
{
    ...
    }
    {
        ...
        if (nvs_open("cam", NVS_READONLY, &nh) == ESP_OK) {
            ...
            if (nvs_get_blob(nh, "pack", staged, &plen) == ESP_OK && plen > PACK_HDR_LEN) {
                ...
                ESP_LOGI(TAG, "stored policy pack version %lu re-applied at boot: %s (cause %u)", (unsigned long)ver, cause ? "REFUSED" : "committed", cause);
            }
            nvs_close(nh);
        }
    }
    camera_config_t cam_cfg = {
        ...
        .pixel_format = PIXFORMAT_GRAYSCALE, .frame_size = (FRAME_W == 640 ? FRAMESIZE_VGA : FRAMESIZE_QVGA), .jpeg_quality = 12,
    ...
    vTaskDelay(pdMS_TO_TICKS(3000));
    {
        ...
        if (sensor) {
            ...
            ESP_LOGI(TAG, "exposure and gain locked: aec_value=%d agc_gain=%d", sensor->status.aec_value, sensor->status.agc_gain);
        }
    }
    ...
    while (1) {
            ...
            if (n_read >= 4 && cmd_buf[0] == 'C') {
                ...
                if (target_node == node_id || target_node == 0xffff) {
                    if (cmd_buf[3] == 'n') {
                        ...
                        uint64_t image_hash_val;
                        uint16_t cause = swap_execute(&ver, &image_hash_val, &us);
                        swp_t sw = { {'S','W','P','1'}, node_id, ver, cause, us, image_hash_val, (uint64_t)esp_timer_get_time() };
        ...
        front_end(&motion_cells, &dark, &step, &door_hit, &scene);
        {
            ...
            if (esp_timer_get_time() - t_fe_log > 10000000) {
                ESP_LOGI(TAG, "front end %dx%d: mean %lld us, max %lld us over %d frames; free PSRAM %u B", FRAME_W, FRAME_H, (long long)(fe_sum / fe_n), (long long)fe_max, fe_n, (unsigned)heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
                fe_sum = 0;
                fe_n = 0;
                fe_max = 0;
