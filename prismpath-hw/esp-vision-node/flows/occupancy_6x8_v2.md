...
Version 2 of the policy: the evidence threshold is lowered from 28 to 20 moving cells, so a close pass by
one person is escalated with a frame. The same grid, the same fields, one constant changed, and the
codebook changes with it (a cut at 20 replaces the cut at 28).
