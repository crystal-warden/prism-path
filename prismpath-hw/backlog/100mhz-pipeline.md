...
- Shipped at **50 MHz**: WNS +1.623, timing clean. Evaluate latency 5 to 21 cycles on
  `incident_severity` ⇒ **100 to 420 ns worst case**.
...
**~6 to 23 cycles at 100 MHz ⇒ roughly 60 to 230 ns worst case**: a true ~2× latency win, not
a wash.
...
2. `make -C tb` sensor replay: **7,436 samples, zero mismatches**, and record the new
   min to max cycle counts in the day log.
