...
static int8_t project_tof(uint16_t mm) {
    if (mm < 100)
        return 0;
    if (mm < 300)
        return 1;
...
static int8_t project_pot(int raw) {
    if (raw < 1024)
        return 0;
    if (raw < 2048)
        return 1;
...
static uint8_t my_slot = 0xFF, my_kind = 0xFF;
...
static int8_t read_own_band(void) {
    if (my_kind == KIND_POT) {
        int raw = 0;
        adc_oneshot_read(adc1, POT_ADC_CHAN, &raw);
        return project_pot(raw);
    }
    if (my_kind == KIND_TOF) {
        if (!tof_ok)
            return 3;
        return project_tof(read_range_single(&tof));
    }
    return 3;
}
...
void app_main(void) {
    ...
    if (my_slot == 0xFF) {
        emit("node %02x:%02x:%02x:%02x:%02x:%02x has NO role in ROLES[] — idle\r\n", mac[0], mac[1], mac[2], mac[3],
             mac[4], mac[5]);
        for (;;)
    ...
    node_id = mac[5];
    if (FUSE_TABLE_A_LEN > TBL_MAX) {
        emit("[%s] fusion table too large\r\n", my_label);
        for (;;)
            vTaskDelay(pdMS_TO_TICKS(1000));
    }
    memcpy(tbl, FUSE_TABLE_A, FUSE_TABLE_A_LEN);
    uint8_t rc = parse_table(FUSE_TABLE_A_LEN);
    active_id = POLICY_ID_A;
    if (rc) {
        emit("[%s] fusion table parse FAILED rc=%u\r\n", my_label, rc);
        for (;;)
            vTaskDelay(pdMS_TO_TICKS(1000));
    }
    ...
    for (;;) {
        ...
        if (uart_read_bytes(UART, &c, 1, 0) == 1) {
            if (c == 'R' || c == 'r')
                ...
                drop_pct = (uint8_t)((c - '0') * 10);
                emit("[%s] interference: dropping %u%% of received verdicts\r\n", my_label, drop_pct);
            } else if (c == 'X' || c == 'x') {
                drop_pct = 99;
                emit("[%s] interference: near-total blackout (99%% drop)\r\n", my_label);
            }
        }
        ...
        if (coord && ack_deadline_us && now >= ack_deadline_us) {
            ...
            if (ack_n >= EXPECT_ACKS) {
                ...
                emit("[%s] coord %d ACKs COMMIT seq=%u flip in %ums\r\n", my_label, ack_n, rollout_seq, FLIP_DELAY);
            } else
                emit("[%s] coord ABORT seq=%u only %d/%d, fleet stays on fusion-%s\r\n", my_label, rollout_seq, ack_n,
                     EXPECT_ACKS, pname(active_id));
            coord = 0;
        }
        ...
        if (now >= next_bcast) {
            next_bcast = now + BCAST_MS * 1000;
            int8_t own = read_own_band();
            bands[my_slot] = own;
            band_seen_us[my_slot] = esp_timer_get_time();
            ...
            int lv;
            if (!strcmp(posture, "CRITICAL") || !strcmp(posture, "TAMPER"))
                lv = 1;
            else if (!strcmp(posture, "DEGRADED") || !strcmp(posture, "WARN")) {
