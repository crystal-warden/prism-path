...
def carr(name, blob):
    hexb = ",\n  ".join(", ".join(f"0x{byte:02x}" for byte in blob[offset:offset + 12]) for offset in range(0, len(blob), 12))
    return f"static const uint8_t {name}[] = {{\n  {hexb}\n}};"


def compile_policy(arm_threshold):
    comp = pc.compile_flow(parse(FUSE_FLOW.replace("{ARM}", str(arm_threshold))), 25)
    got = {field: comp.fields[field] for field in EXPECT_FIELDS if field in comp.fields}
...
def mac_bytes(mac):
    return "{" + ", ".join(f"0x{int(octet, 16):02x}" for octet in mac.split(":")) + "}"


def main() -> int:
    table_a = compile_policy(2)          # policy A: arm >= 2 (permissive)
    table_b = compile_policy(3)          # policy B: arm >= 3 (tightened)
    ida, idb = fnv1a32(table_a), fnv1a32(table_b)

    roles_c = ",\n  ".join(
        f"{{ {mac_bytes(mac)}, {slot}, {kind}, \"{label}\" }}" for mac, slot, kind, label in ROLES)
    verd_c = ", ".join(f'"{verdict}"' for verdict in VERDICTS)
    out = [
        ...
        carr("FUSE_TABLE_A", table_a),
        f"static const uint16_t FUSE_TABLE_A_LEN = {len(table_a)};",
        f"static const uint32_t POLICY_ID_A = 0x{ida:08x}u;",
        "",
        "/* policy B (tightened): CRITICAL arms at knob band >= 3 */",
        carr("FUSE_TABLE_B", table_b),
        f"static const uint16_t FUSE_TABLE_B_LEN = {len(table_b)};",
    ...
    print(f"wrote fusion_mesh_table.h: A={len(table_a)}B id_A=0x{ida:08x} (arm>=2) | B={len(table_b)}B id_B=0x{idb:08x} (arm>=3)")
