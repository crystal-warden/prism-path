...
- an **opcode-whitelist walk** over the image's atom section — the decidable-fragment membership
  re-check. The compiler already guarantees fragment membership at build time (it rejects
  non-Level-M constructs with stable `SubsetError` reasons); the walk re-verifies the shipped
