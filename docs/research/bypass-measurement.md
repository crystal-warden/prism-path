...
- Mechanical strata: **high**: likely 0.8 to 1.0. The floor does no normalization whatsoever today.
...
| `zero_width` | mechanical | **0.00 to 0.05** | invisible-character stripping is exact and lossless |
| `diacritics` | mechanical | **0.00 to 0.05** | combining-mark removal is exact and lossless |
| `homoglyph` | mechanical | **0.00 to 0.10** | confusable mapping is a finite table; misses are table gaps, not ambiguity |
| `spacing` | mechanical | **0.05 to 0.30** | collapsing single-character-separated runs is a heuristic, not a fold |
| `punctuation` | mechanical | **0.05 to 0.30** | same heuristic; interior punctuation overlaps legitimate tokens |
| `leetspeak` | mechanical | **0.10 to 0.40** | **lossy**: `1`→`l`/`i` and `0`→`o` are ambiguous, so only unambiguous substitutions are safe to fold |
| all `semantic` | semantic | **0.95 to 1.00** | must be *unchanged*; normalization is not expected to touch paraphrase |
...
|---|---|---|---|
| `paraphrase` | 1.00 | **0.20 to 0.70** | the case embeddings are built for; largest expected drop |
| `euphemism` | 1.00 | **0.30 to 0.80** | community terms may sit far from the exemplar language |
| `roleplay` | 1.00 | **0.40 to 0.90** | surface is *about fiction*; intent is buried under framing |
| `translation` | 1.00 | **0.80 to 1.00** | **bge-base is an ENGLISH model.** Non-English text embeds poorly, so little drop is expected. Predicted explicitly so a good result cannot be claimed as foresight |
| all mechanical | 0.00 to 0.91 | **unchanged** | P1 must not alter P0's behaviour; composition is union-only |
...
traded a published failure for an unpublished, worse one. Any nonzero result here blocks the
...
Run 1 measured the mechanical rollup at **0.79**. The pre registered band (§5.1) was **0.8 to 1.0**.
...
| `case` | 0.00 | **0.00** | hit |
| `zero_width` | 0.00 to 0.05 | **0.00** | hit |
| `diacritics` | 0.00 to 0.05 | **0.00** | hit |
| `homoglyph` | 0.00 to 0.10 | **0.00** | hit |
| `leetspeak` | 0.10 to 0.40 | **0.64** | **MISS (high)** |
| `punctuation` | 0.05 to 0.30 | **0.73** | **MISS (high)** |
| `spacing` | 0.05 to 0.30 | **0.91** | **MISS (high)** |
| semantic ×4 | 0.95 to 1.00 | **1.00** | hit |
