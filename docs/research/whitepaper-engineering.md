...
not one measured across the full matrix of torch/ONNX/fp16/CPU–GPU builds. A legitimate cross-platform
...
Routers are swappable: `EmbeddingRouter` (cheap, flow-dependent — ~0.52–0.81 per stratum, 0.69
...
- **Errors** (the flow won't run correctly): undefined start / edge targets; an unsafe or
  ...
  end at `max_steps`). `validate` exits non-zero on any error.
