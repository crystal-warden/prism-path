...
### #118 — the FPGA Zeckendorf codec measured on silicon: encoder and decoder in the PL fabric, byte-exact and 1,000,000/1,000,000 round-trips on the Zynq (August 2026)

**Claim:** the shift-register half of Phase C2 — #81's "hardware shift-register codec … not built", the back half of #104's front-half MCU encoder — now RUNS in the FPGA fabric, encoder AND decoder, with no processor in the codec path, measured byte-exact against the same Python reference the MCU bench used. On the physical Zynq the fabric's encoded wire is bit-identical to the reference and its decode inverts it: decode(encode(n)) == n for 1,000,000/1,000,000 values. This closes C2.

**Method:** the existing bit-serial encoder `prismpath-hw/rtl/zeck_enc.sv` plus a new exact-inverse decoder `zeck_dec.sv`, wired mouth-to-ear in `zeck_codec_core.sv` — a PEEK path encodes one value and reads back both the emitted wire bits and the round-trip decode, and a self-test sweeps n=1..MAX counting decode(encode(n))==n entirely in the PL — wrapped in an AXI4-Lite face `zeck_codec_axi.sv` (MAGIC "ZCK1") as the `ppt_zeck` overlay (`vivado/build_overlay_zeck.tcl`, PS7 + codec only, no board XDC; Vivado 2023.2, xc7z020clg400-1, 50 MHz, all timing met, WNS +11.9 ns, 962 LUTs). Conformance in cocotb/verilator against `adapters/telemetry/` (`zeckendorf.py`, `packed.py`), the same seed-42 corpus #104 verified (64 typical + 64 stress events). On the board (`codec-bench/measure_zeck_codec.py`, PYNQ, reached over the Protectli jump to an Arty Z7-20): the in-fabric self-test sweep, plus a PEEK read-back of the fabric's encoded wire per corpus value compared byte-for-byte against an inlined Zeckendorf reference (itself cross-checked 0-mismatch against `zeckendorf.encode_stream` over 1..2000). A frame-level receiver `zeck_frame_rx.sv` (built on `zeck_dec`, gap-delimited, cocotb 10/10 on real walker frames) carries the codec onto a live Facet wire; the end-to-end walker integration is off-repo bench material.

**Result:** on the physical Zynq-7020: **1,000,000 / 1,000,000 in-fabric round-trips decode(encode(n)) == n, first_fail = 0, err = 0**, and **225 / 225** distinct seed-42 corpus wire ints **bit-exact vs the reference AND decode == n**. Simulation: encoder 128/128, decoder 128/128 (512 wire ints), codec core 64/64 PEEK + 300/300 self-test. Measuring on silicon caught a real Vivado-only defect the sim-only history had hidden: the Fibonacci ROM authored as an initial-block loop `fib[j] = fib[j-1] + fib[j-2]` simulates correctly (sequential) but Vivado evaluated the RHS against the un-updated array, zeroing `fib[2..]` — the board passed n = 1, 2 (which use the directly-assigned `fib[0]`, `fib[1]`) and failed from n = 3; the fix is an explicit constant `localparam` table. The pre-existing `zeck_enc.sv` had carried that same latent bug the whole time, invisible because it had only ever been simulated — exactly the modeled-vs-measured gap this row closes. **Honest scope:** one board, one bring-up; the codec is timing-clean at 50 MHz with generous headroom but is NOT throughput-benchmarked on silicon; the measurement domain is the seed-42 corpus plus a 1 M round-trip sweep (codes to ~F30 exercised; the F2..F46 ROM covers more), broad but not exhaustive; the board is reached over the Protectli jump and the bitstream is an off-repo lab artifact regenerated from `rtl/` + the TCL, not committed; the OTS priority-date anchor over the on-silicon result is owner-gated (stamped at publish). Closes #81's "hardware shift-register codec (C2) … not built" (the C4 vector-quantization tier remains open) and is the back half of #104's front-half MCU encoder.

**Provenance:** `prismpath-hw/rtl/zeck_enc.sv` (the fib-ROM fix), `zeck_dec.sv`, `zeck_codec_core.sv`, `zeck_codec_axi.sv`, `zeck_codec_top.v`, `zeck_frame_rx.sv`; `prismpath-hw/codec-bench/rtl-tb/` (`test_zeck_dec.py`, `test_zeck_codec.py`, `test_zeck_frame_rx.py`, `Makefile.{dec,codec,frame}`); `prismpath-hw/codec-bench/measure_zeck_codec.py`; `prismpath-hw/codec-bench/rtl-codec.md`; `prismpath-hw/vivado/build_overlay_zeck.tcl`; Vivado 2023.2 / xc7z020clg400-1; a PYNQ Arty Z7-20 reached via the Protectli jump; reference `adapters/telemetry/{zeckendorf,packed}.py` and the seed-42 corpus of #104.
...
**Result:** **BOTH architectures, same day, byte-identical frozen corpus** (sha `a24026c7...` verified on each host). aarch64 (the GX10): **624/624 events, 49/49 streams clean, fail-safe PASS** (uninitialized state plus one event lands on lockdown, not normal); **624/624 receipts complete, 0 decision mismatches, 0 non-monotonic timestamps, 0 nonzero causes on clean commits**, session Merkle root `58c025ba364c0c7427cb8aa0374bf2c8f45b9780a0c315469fb9c98d2479fa56`. x86_64 (the Protectli): the identical result set (624/624 + fail-safe PASS, 624/624 receipts, 0/0/0), session root `c92517fa62152079b8808b4a5c11c0bd9346082337b6b678858109d83797e52c`. policy_hash `207748442a7915c8` on both. Each session anchored under its own SHA256SUMS + OTS stamp (pending Bitcoin at stamp time). **Honest scope:** receipt roots are per-session by design (the signed time axis), so each anchor covers ITS certified session, the like-for-like closure of #119's caveat under that row's own convention that transient captures freeze a verdict record; the per-arch discipline is satisfied in-row (both arches, one day) rather than deferred; the cause byte on kernel receipts is the FORMAT plus the clean-commit stamp — nonzero kernel causes arrive with the paths that produce them (loader migration receipts, refusal emissions), a named follow-on. #119's receipt-root caveat is annotated *(Closed by #128.)* per LEDGER_STANDARDS §3 at this fold.

**Provenance:** `prismpath-ebpf/` (`ppt_common.h`, `ppt_select.bpf.c`, `receipts_selector.c` with the cause check, `evidence/receipts_session_2026-08-28.{log,SHA256SUMS,SHA256SUMS.ots}` for aarch64 and `evidence/receipts_session_x86_64_2026-08-28.{log,SHA256SUMS,SHA256SUMS.ots}` for x86_64); run on each host: `clang -target bpf` build, then `sudo ./cert_selector selector_corpus.bin ppt_select.bpf.o` and `sudo ./receipts_selector selector_corpus.bin ppt_select.bpf.o`.
...
- **v2.4** (August 2026): rows #117 to #120 folded: the pure-fabric control plane
  ...
  migration, and audit receipts, certified 624/624 in-kernel and deployed live on a veth (#119),
  and the spiral mesh integrity soak under real 2.4 GHz interference, 0 wrong symbols across 8
  phases (#120). This anchor also covers the second draft of the Facet / Figueroa quantization
...
- **v2.3** (August 2026): rows #109 to #116 folded in from staging — the per-evaluate WCET formula
  ...
  the interpreter ported to a second FPGA family (Lattice ECP5) on a fully open toolchain, the
  decision loop run entirely in fabric with the loader exiting, and the spiral packing profile
  carried machine to machine from a lint gated bake to three braided Facet streams over real
  ESP-NOW RF. Per §3, #101's plain code per-channel projection caveat is reconciled by #116
  (quantizers derived from the signed policy, proven over the air). The underlying evidence sets are
...
- **v2.7** (August 2026): row #128 folded from staging — the kernel receipt gains the cause
  code field (former pad slot, byte-compatible with all historical receipts) and the #119
  receipt-root caveat closes: fresh certified sessions on BOTH architectures (GX10 aarch64 +
  Protectli x86_64, byte-identical frozen corpus verified per host), each session's Merkle
  root OTS-anchored and Bitcoin-complete. #119's two pending-anchor caveats annotated
  *(Closed by #128.)* per §3. Anchored in
  `prismpath/evidence/ledger_v2.7_2026-08-28.SHA256SUMS` (`.ots` alongside); the anchor, not
  this prose, is the authoritative timestamp.
- **v2.8** (September 2026): rows #129 to #143 folded from staging. The August rows #129 to #139
  (fabric cause register, receipt streams, migration receipts across loader, forwarder, one shot CLI,
  and the unified trail, the ESP-NOW field walk, the GRC engine's breadth, dogfood, and evidence typed
  verdicts) as their sessions wrote them; #140 and #141, the Figueroa quantization formal development
  in Lean 4 and the three quantizer defects it found, rewritten once from the finished state in place of
  four phase by phase drafts; #142 and #143, the pre registered comparison against OPA, Cedar, Cerbos,
  OpenFGA, and Openlane across all six phases and its hardware legs, rewritten once in place of six.
