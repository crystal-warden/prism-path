...
def test_applicability_scoping_matches_statutory_reach():
    _std()
    counts = {actor: ca.applicability_determination(actor)["counts"]["applicable"]
              for actor in ("commercial", "healthcare", "government", "gov_vendor")}
    ...
    assert na and all(determination["status"] == "not-applicable" and determination["reason"] for determination in na)
...
def test_seven_config_objectives_are_machine_checkable_and_scanned():
    ...
    facts = {dc.FACT_KEYS[objective_id]: True for objective_id in cfg}
...
def test_crosswalk_references_are_valid_both_sides():
    ...
    for edge in cw["edges"]:
        assert edge["a"] in tx, f"bad source {edge['a']}"
        covered.add(edge["a"])
        for target_control in edge["b"]:
            assert target_control in aig, f"bad target {target_control}"
...
def test_policy_specs_reference_real_objectives():
    tx = json.load(open(os.path.join(HERE, "catalog", "texas_ai.json")))["controls"]
    allobj = {objective["id"] for control in tx.values() for objective in control["objectives"]}
    specs = glob.glob(os.path.join(HERE, "sop_specs", "texas_*.json"))
    assert specs, "expected Texas SOP specs"
    for spec_path in specs:
        spec = json.load(open(spec_path))
        assert spec["standard"] == "texas_ai"
        for control_id in spec["controls"]:
            assert control_id in tx, f"{spec['doc_id']} references unknown control {control_id}"
        for sec in spec["sections"]:
            for oid in sec.get("objectives", []):
                assert oid in allobj, f"{spec['doc_id']} references unknown objective {oid}"
...
def test_runtime_connector_derives_config_facts_from_real_engine_runs():
    ...
    stops = {receipt["stopped"] for receipt in receipts}
...
def test_standard_notice_marks_unvalidated():
    ...
    assert notice["validated"] is False and "REVIEW ASSISTANCE" in notice["notice"]
...
def test_documented_plane_is_complete():
    ...
    tx = json.load(open(os.path.join(HERE, "catalog", "texas_ai.json")))["controls"]
    docobj = {objective["id"] for control in tx.values() for objective in control["objectives"] if objective["mechanism"] == "documented"}
    covered = set()
    for spec_path in glob.glob(os.path.join(HERE, "sop_specs", "texas_*.json")):
        for sec in json.load(open(spec_path))["sections"]:
