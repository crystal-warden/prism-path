...
SEVEN = {"3.1.8", "3.1.10", "3.1.11", "3.5.3", "3.5.7", "3.5.8", "3.13.11"}
...
def test_machine_checkable_controls_are_the_seven_seeded():
    _std()
    assert set(pc.machine_checkable_controls()) == SEVEN


def test_required_facts_match_the_check_registry():
    _std()
    req = pc.required_facts()
    assert set(req) == SEVEN
...
def test_partial_posture_defers_what_it_cannot_decide():
    ...
    assert set(res["deferred"]) == SEVEN - {"3.13.11", "3.1.8"}
