...
def test_live_end_to_end_emits_valid_standards():
    ...
    recs = []
    for filename in sorted(os.listdir(reqdir)):
        req = ca.load_request(os.path.join(reqdir, filename))
        control = ca.get_control(req["control_id"])
        det = ca.adjudicate(control, req)
        assert det is not None, filename
        assert det["status"] in ("met", "partially-met", "not-met")
        recs.append(ca.result_record(control, req, det, ca.attest(control, req, det)))
    out = ca.emit_reports(recs, fmt="both")
    for name, emission in out.items():
        assert emission["valid"], (name, emission["errors"][:3])
...
def test_rev3_adjudication_runs_end_to_end():
    ...
    try:
        control = ca.get_control("3.1.1")                            # Rev 3 Account Management
