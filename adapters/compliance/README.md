...
`docs/papers/SUPPORTING_EVIDENCE.md`.

Runs against the published `prismpath` core (`import prismpath`). Test env:
`~/cwprojects/prismpath/.venv`. Live adjudication calls Gemma at `http://127.0.0.1:8888`.
...
| file | role |
|---|---|
| `compliance_adapter.py` | the adapter: Retrieval (selectable catalog), Ingestion, Adjudicator (escalation-default, method-profile-aware), Sink, Attestation (reuses core `ledger_airgap`), Deferral (HITL + discovery) |
...
| `semantic_retrieve.py` | EmbeddingGemma (CPU) retrieval map; **run under `~/jupyterlab/.venv`** |
...
$PP -m pytest -q                    # deterministic adapter suite (~126)
$PP -m pytest -m gemma -q           # opt-in live-gemma integration
$PP -m prismpath.cli validate flows/nist_800171_generic.md
python3 build_catalogs.py           # rebuild the dual catalogs
```
