...
**Still open**, both day scale, in the order I would take them:

1. `symbolAlg = symbolCount` as a theorem. Checked by evaluation on ranges and on the corpus today.
   The proof needs: fine grid cells are consecutive intervals whose boundaries are exactly
   `boundaries consts` (an empty gap drops a cell but not a boundary), each atom is constant on a fine
   cell, and the merge keeps a boundary exactly when the truth vector differs across it. List
   recursion over `Option Int` bounds is the cost.
2. The spiral band map as a bijection from the product of cells onto indices, and band equality
