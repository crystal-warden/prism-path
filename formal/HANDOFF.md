...
1. The spiral band map as a bijection from the product of cells onto indices, and band equality
   implying route equality (`adapters/telemetry/spiral.py`). The core is that the mixed radix Gray
   enumeration is a permutation of the product; parity with the frozen `spiral_fusion.json` cell to
   index table is the bridge.

The algorithm equals canonical theorem (`FQ/AlgEq.lean`) closed the other item. Its route: the fine
grid is a contiguous chain of integer intervals (`Contig`) whose interiors hold no boundary, so every
atom is constant on a fine cell; the merge keeps a cell start exactly when the truth vector changes
across it (`go_spec`), so the merged starts are the retained boundaries as a set; the first
containing cell of a contiguous chain is the count of starts at or below the value
(`findIdx_contig`); two duplicate free lists with the same members have equal counts.
