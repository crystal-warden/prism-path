...
## 0. Status (September 2026)

Executed in the handing off session; this document is now half record, half plan. Read `formal/FQ/`
first. `lake build` is clean, zero `sorry`, and every theorem below depends only on `propext`,
`Classical.choice`, `Quot.sound` (`FQ/Axioms.lean`).

| theorem | file | statement |
|---|---|---|
| `decision_preservation` | I1.lean | I1: equal quantization implies equal route, partitions derived from the policy, well typed total readings |
| `symbolCount_eq_truthVec` | I1.lean | the numeric core: equal canonical symbol implies equal atom truth vector |
| `coarsest_interval` | I1.lean | I1b: every retained boundary separates adjacent cells with different truth; coarsest among interval partitions |
| `reconstruct_route` | Reconstruct.lean | I1 in words: routing the reconstructed representative reproduces the decision |
| `Zeck.decode_encode` | Zeckendorf.lean | the Fibonacci code round trip for positive integers, on Mathlib's Zeckendorf theorem |
| `Zeck.takeCode_encode` | Zeckendorf.lean | I2 self framing: the first `11` after `encode n` closes exactly `encode n` |
| `Zeck.decodeStream_flatMap` | Zeckendorf.lean | a concatenation of codes decodes to exactly the values, no length header |
| `symbolAlg_eq_symbolCount` | AlgEq.lean | the reference algorithm (fine grid, merge, first containing cell) equals the canonical boundary count on every integer |
...
| `Spiral.spiral_bijection` | Spiral.lean | the route bucketed layout is a bijection between cells and indices |
| `Spiral.band_route`, `Spiral.band_eq_route_eq` | Spiral.lean | a band determines the route of its cells |

Bridge to the code (`FQ/Bridge.lean` by hand, `FQ/Vectors.lean` generated by `gen_vectors.py`, all
`#guard`, evaluated at build): the reference partition algorithm and the canonical form agree on
integer ranges; 35 well typed Level M predicate cases match the frozen corpus; 90 frozen routes and
164 symbols equal the corrected reference quantizer's on every decisions.json reading; 34 spiral
probes; 300 Fibonacci codes equal the reference's bit strings.

**Stating the theorem found three defects in the reference quantizer** and its Rust mirror, all in
atom forms the frozen corpus never exercised: numeric `in` and `not in` constants were not cut
points, the truthiness cut at zero was omitted when other constants existed, and truthiness on a
string field had no `""` constant. Each violated I1 on a one line policy. Fixed on branch
`fix/quantizer-cut-points` (merged here), decisions corpus v2, ledger rows #140 and #141.

**Nothing open.** The spiral bijection (`FQ/Spiral.lean`) closed the last item: the reflected Gray
enumeration is a permutation of the product (`gray_perm_product`), bucketing by route is a permutation
of the enumeration (`layout_perm`), so the layout is duplicate free and complete and index of cell and
cell of index invert (`spiral_bijection`); the band of an index determines the route of its cell
(`band_route`). The bridge derives the fusion layout inside Lean from the policy (radices from the
partitions, routes from the rules, cell routes from the representatives) and matches all 108 frozen
cells and the reference's Gray order. The development is frozen; `formal/README.md` is the plain
statement of what it proves and what it does not.

Also outside the model by decision, and worth saying in any writeup: null semantics (72 of the 136
Level M vectors are missing field cases), cross kind comparisons (13 vectors), and readings with
floats (the reference casts with `int()`).
...
The agy drafted, uncompiled statement was removed once the built files superseded it. Its one
lesson is kept: it stated the main theorem with the partition list as a free parameter, which made
the theorem false rather than unproven, and no compiler catches that.
