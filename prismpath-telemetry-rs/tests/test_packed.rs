...
fn test_round_trip() {
    ...
    for word_bits in [8, 64] {
        for ints in &test_cases {
            assert_eq!(p::decode(&p::encode(ints, word_bits)), *ints);
        }
    }
}
...
fn test_output_is_whole_words() {
    for word_bits in [8, 64] {
        ...
        let wire = p::encode(&ints, word_bits);
...
fn test_bits_survive_packing() {
    let bits = z::encode_stream(&[4, 2, 7]);
...
fn test_padding_amortizes() {
    ...
    let small = p::padding_overhead(&ints_small, 64);
    let big = p::padding_overhead(&ints_big, 64);
...
fn test_wire_is_dense() {
    ...
    let bits = z::encode_stream(&ints).len();
    let wire = p::encode(&ints, 64);
